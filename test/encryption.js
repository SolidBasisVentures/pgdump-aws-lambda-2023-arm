...
describe('encryption', () => {
	...
	it('should encrypt and decrypt', async () => {
		...
		// decrypt
		const decryptedStream = encryption.decrypt(fs.createReadStream(encryptedPath), ENCRYPT_KEY, IV)
		...
		expect(fs.readFileSync(decryptedPath).toString('utf8')).to.equal('some-unencrypted-data')
	})

	it('should throw an error for an invalid key', () => {
		expect(() => encryption.encrypt(undefined, 'bad-key', 'bad-IV')).to.throw(
			'encrypt key must be a 32 byte hex string',
		)
	})

	it('should throw an error for an invalid iv', () => {
		expect(() => encryption.encrypt(undefined, ENCRYPT_KEY, 'bad-IV')).to.throw(
			'encrypt iv must be exactly 16 bytes, but received 6',
		)
	})
})
