...
describe('Handler', () => {
	...
	it('should upload a backup', async () => {
		...
		expect(result).to.equal('mock-uploaded/2017-05-02/dbname-02-05-2017@01-33-11.backup')
	})
	...
	it('should upload the backup file and an iv file', async () => {
		...
		const event = {
			...mockEvent,
			ENCRYPT_KEY: '4141414141414141414141414141414141414141414141414141414141414141',
		}
	...
	it('should throw an error when PGDATABASE is not provided', () => {
		...
		return handler(event).should.be.rejectedWith(/PGDATABASE not provided in the event data/)
	})
	...
	it('should throw an error when S3_BUCKET is not provided', () => {
		...
		return handler(event).should.be.rejectedWith(/S3_BUCKET not provided in the event data/)
	})
	...
	it('should handle pgdump errors correctly', () => {
		...
		return handler(mockEvent).should.be.rejectedWith(/pg_dump gave us an unexpected response/)
	})
})
